...
protected function extractUrls($content)
{
    ...
    // Handle requirejs-config.js and other critical files explicitly
    $criticalJsPatterns = [
        '/\/static\/(?:version\d+\/)?requirejs\/require\.js/i',
        '/\/static\/(?:version\d+\/)?requirejs-config\.js/i',
        '/\/static\/(?:version\d+\/)?mage\/requirejs\/mixins\.js/i',
    ];
    
    foreach ($criticalJsPatterns as $pattern) {
        preg_match_all($pattern, $content, $matches);
        if (!empty($matches[0])) {
    ...
    // Process inline JSON that might contain URLs
    preg_match_all('/"(\/static\/[^"]+\.(?:js|css|png|jpg|jpeg|gif|svg))"/i', $content, $jsonMatches);
